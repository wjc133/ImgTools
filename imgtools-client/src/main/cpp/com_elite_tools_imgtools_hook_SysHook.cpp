#include <windows.h>
#include "com_elite_tools_imgtools_hook_SysHook.h"
#include <jni.h>
#include <iostream>
...
JNIEXPORT void JNICALL Java_com_elite_tools_imgtools_hook_SysHook_registerHook(JNIEnv *env, jobject obj, jobject kl)
{
    std::cout<<"invoked....";
  ...
  {
     std::cout<< "C++: Java_SysHook_registerKeyHook - Hook failed!"<<"\n";
    return ;
  }
  else
  {
    std::cout<< "C++: Java_SysHook_registerKeyHook - Hook successful"<<"\n";
  }
...
JNIEXPORT void JNICALL Java_com_elite_tools_imgtools_hook_SysHook_unRegisterHook(JNIEnv *env, jobject obj)
{
  if (hookThreadId == 0)
